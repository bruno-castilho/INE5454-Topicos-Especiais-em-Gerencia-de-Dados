...
function somaPrecos(lista) {
  ...
  return lista.reduce((total, item) => total + parsePreco(item.PRECO_PRINCIPAL), 0);
}
...
function analisarPrecos(marcas) {
 ...
 function Analiseprecos(props) {
    ...
    const { primery, secudary, max, growh, intra  } = props;
    //let data = [];
    ...
    useEffect(() => {
     ...
     console.log(analise.totalProGrow)
      console.log(analise.totalProMax)
      console.log(analise.totalProIntegra)
    const data = [
    ...
    return(
        ...
        >
            ...
            <Typography variant="body1" sx={{ mt: 3, fontSize: 14, fontFamily: "Afacad Flux, sans-serif" }}>
