...
import { maxPrices } from '../../../api/max-prices'
...
export function MaxChart() {
  ...
  const { data } = useQuery({
    queryKey: ['maxPrices', brands?.length === 0 ? undefined : brands],
    queryFn: () =>
      maxPrices({
  ...
  const brandsData = Object.keys(data?.maxPrices ?? [])
  const series = brandsData.reduce((currentSeries, brand) => {
    Object.keys(data.maxPrices[brand]).forEach((category) => {
      if (!currentSeries[category]) currentSeries[category] = []

      currentSeries[category].push(data.maxPrices[brand][category])
    })
